    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(argument("url", StringArgumentType.greedyString()).executes(ctx -> {
            ...
            for (int index = 0; index < imageToBuild.getHeight(); index++) {
                for (int i = 0; i < imageToBuild.getWidth(); i++) page.append("{\"text\":\"").append(block).append("\",\"color\":\"#").append(Integer.toString(imageToBuild.getRGB(i, index) & 0xFFFFFF | 0xF000000, 16).substring(1)).append("\"},");
                page.append("{\"text\":\"\\\\n\"},");
            }
            try {
                stack.getOrCreateNbt().copyFrom(StringNbtReader.parse("{title:\"\",author:\"ImageBook\",pages:['[" + page.substring(0, page.length() - 1) + "]']}"));
